...
type PositionReq struct {
	g.Meta    `path:"/backend/position/add" tags:"手工位" method:"post" summary:"创建手工位接口"`
...
type PositionUpdateReq struct {
	...
	PicUrl    string `json:"pic_url" v:"required#手工位图片链接不能为空" dc:"图片链接"`
	Link      string `json:"link"    v:"required#手工位跳转链接不能为空" dc:"跳转链接"`
	Sort      int    `json:"sort"    dc:"排序"`
	GoodsName string `json:"goods_name"    v:"required#商品名称不能为空" dc:"商品名称"`
	GoodsId   int    `json:"goods_id"    v:"required#商品ID不能为空" dc:"商品ID"`
}
